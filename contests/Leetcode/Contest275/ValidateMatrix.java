...
public class ValidateMatrix {
    private static void solve() throws IOException {
        ...
        ValidateMatrix sol = new ValidateMatrix();
        System.out.println(sol.checkValid(new int[3][3]));

    }

    public boolean checkValid(int[][] matrix) {
        for (int i = 0; i < matrix.length; i++) {
            List<Integer> rowList = createList();
            List<Integer> colList = createList();
            for (int j = 0; j < matrix.length; j++) {
                rowList.remove(matrix[i][j] - 1);
                colList.remove(matrix[j][i] - 1);
            }
