...
public class DivideStringToGroupsOtherSol {
    private static void solve() throws IOException {
        DivideStringToGroupsOtherSol obj = new DivideStringToGroupsOtherSol();
        String[] ans = obj.divideString("abcdefgh", 3, 'x');
        for (String i : ans) {
    ...
    public String[] divideString(String s, int k, char fill) {
        ...
        String[] ans = stringLength % k == 0 ? new String[stringLength / k] : new String[(stringLength / k) + 1];

        // 1. First append the string to string buffer
        StringBuffer sb = new StringBuffer(s);

        // 2. check and append fill
        if (sb.length() % k != 0) {
            for (int i = 0; i < k - (stringLength % k); i++) {
                sb.append(fill);
            }
        }
        // 3. split string into parts
